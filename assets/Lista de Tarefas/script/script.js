...
function addMeta() {
    ...
    if (input.value.trim() === '') {
        errorMessage.textContent = 'A tarefa não pode ser vazia.';
        return;
    }
...
function metasRealizadas() {
    ...
    if (realizadas.length === 0) {
        alert('Não existem tarefas realizadas!');
    } else {
        let mensagem = `Tarefa realizadas (${realizadas.length}):\n`;
...
function metasAbertas() {
    ...
    if (abertas.length === 0) {
        alert('Não existem tarefas em aberto!');
    } else {
        let mensagem = `Tarefas em aberto (${abertas.length}):\n`;
...
function deletarMetas() {
    ...
    if (deletarOpcoes.length === 0) {
        alert('Não existem tarefas cadastradas para deletar!');
        return;
    }

    let mensagem = 'Selecione a tarefa que deseja deletar (Digite o número correspondente):\n';
